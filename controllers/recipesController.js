...
const createNewRecipe = asyncHandler(async (req, res) => {
    const { user, name, categories, ingredients, currency, totalCost, servings} = req.body;
    // ,iamount,iunit
    ...
    try {
        // Create and store the new user 
        const newRecipe = new Recipe({ user, name, categories, currency, ingredients, totalCost, servings });
...
const updateRecipe = asyncHandler(async (req, res) => {
    const { id,user, name, categories, ingredients, currency, servings, totalCost } = req.body;
...
const deleteRecipe = asyncHandler(async (req, res) => {
    ...
    // const menu = await Menu.findOne({ 'breakfast.recipe': { $in: [id] } }).lean().exec();
    //if (menu) {
    //    return res.status(400).json({ message: 'Recipe has assigned menu, please delete the menu first' });
    //}
